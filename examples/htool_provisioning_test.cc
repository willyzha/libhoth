...
TEST_F(HtoolProvisioningTest, WriteSuccess) {
  ...
  EXPECT_CALL(mock_, send(_, _, _))
      .Times(3)
      .WillRepeatedly([&](struct libhoth_device*, const void* req, size_t size) {
        const auto* hoth_req = static_cast<const struct hoth_host_request*>(req);
        EXPECT_EQ(hoth_req->command,
                  HOTH_BASE_CMD(HOTH_PRV_CMD_HOTH_PROVISIONING_LOG));
        const auto* log_req = reinterpret_cast<
            const struct hoth_provisioning_log_request*>(
            static_cast<const uint8_t*>(req) + sizeof(struct hoth_host_request));
        if (call_count == 0) {
          EXPECT_EQ(log_req->operation, PROVISIONING_LOG_WRITE);
          EXPECT_EQ(log_req->offset, 0);
          EXPECT_EQ(log_req->size, 1004);
        } else if (call_count == 1) {
          EXPECT_EQ(log_req->operation, PROVISIONING_LOG_WRITE);
          EXPECT_EQ(log_req->offset, 1004);
          EXPECT_EQ(log_req->size, 496);
        } else if (call_count == 2) {
          EXPECT_EQ(log_req->operation, PROVISIONING_LOG_COMMIT);
          EXPECT_EQ(log_req->size, 1500);
        }
        call_count++;
        return LIBHOTH_OK;
      });
...
TEST_F(HtoolProvisioningTest, ActivateSuccess) {
  struct htool_invocation inv{};
  std::string hex_id(64, 'a');
  EXPECT_CALL(invocation_mock_, GetParamString("device_id", _))
      .WillOnce(DoAll(SetArgPointee<1>(hex_id.c_str()), Return(0)));
  ...
  EXPECT_CALL(mock_, send(_, _, _))
      .WillOnce([&](struct libhoth_device*, const void* req, size_t size) {
        const auto* log_req = reinterpret_cast<
            const struct hoth_provisioning_log_request*>(
            static_cast<const uint8_t*>(req) + sizeof(struct hoth_host_request));
        EXPECT_EQ(log_req->operation, PROVISIONING_LOG_ACTIVATE);
        EXPECT_EQ(log_req->size, 32);
        return LIBHOTH_OK;
      });
  ...
  ASSERT_EQ(htool_provisioning_activate(&inv), 0);
}

TEST_F(HtoolProvisioningTest, ActivateInvalidDeviceId) {
  struct htool_invocation inv{};
  EXPECT_CALL(invocation_mock_, GetParamString("device_id", _))
      .WillOnce(DoAll(SetArgPointee<1>("010203"), Return(0)));

  ASSERT_EQ(htool_provisioning_activate(&inv), -1);
}

TEST_F(HtoolProvisioningTest, LoadMldsaKeyHexSuccess) {
  struct htool_invocation inv{};
  std::string hex_key(HOTH_SECRET_PROVISIONING_MLDSA44_PUBLIC_KEY_BYTES * 2,
                      '4');
  EXPECT_CALL(invocation_mock_, GetParamString("key", _))
      .WillOnce(DoAll(SetArgPointee<1>(""), Return(0)));
  EXPECT_CALL(invocation_mock_, GetParamString("hex", _))
      .WillOnce(DoAll(SetArgPointee<1>(hex_key.c_str()), Return(0)));
...
TEST_F(HtoolProvisioningTest, LoadMldsaKeyFileSuccess) {
  ...
  std::vector<uint8_t> key_data(
      HOTH_SECRET_PROVISIONING_MLDSA44_PUBLIC_KEY_BYTES, 0x44);
...
TEST_F(HtoolProvisioningTest, LoadMldsaKeyInvalidParams) {
  struct htool_invocation inv{};
  EXPECT_CALL(invocation_mock_, GetParamString("key", _))
      .WillOnce(DoAll(SetArgPointee<1>(""), Return(0)));
  EXPECT_CALL(invocation_mock_, GetParamString("hex", _))
