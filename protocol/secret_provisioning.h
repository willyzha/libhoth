...
#define HOTH_SECRET_PROVISIONING_MLDSA44_PUBLIC_KEY_BYTES 1312

struct hoth_secret_provisioning_load_key_args {
  uint16_t offset;
  uint16_t size;
} __attribute__((packed));

#define HOTH_SECRET_PROVISIONING_LOAD_KEY_CHUNK_MAX_SIZE            \
  (LIBHOTH_MAILBOX_SIZE - sizeof(struct hoth_host_request) -         \
   sizeof(struct hoth_secret_provisioning_request_header) -          \
   sizeof(struct hoth_secret_provisioning_load_key_args))

// Loads the ML-DSA-44 public key used to authenticate provisioning payloads.
