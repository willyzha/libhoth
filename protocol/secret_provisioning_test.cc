...
TEST_F(LibHothTest, LoadMldsaPublicKeySuccess) {
  const std::vector<uint8_t> key(
      HOTH_SECRET_PROVISIONING_MLDSA44_PUBLIC_KEY_BYTES, 0x3c);

  int chunk_index = 0;
  EXPECT_CALL(mock_, send)
      .Times(2)
      .WillRepeatedly([&](const void* request, size_t request_size) {
        if (chunk_index == 0) {
          EXPECT_EQ(request_size,
                    sizeof(struct hoth_host_request) +
          ...
          const auto* args = reinterpret_cast<
              const struct hoth_secret_provisioning_load_key_args*>(
              static_cast<const uint8_t*>(request) +
              sizeof(struct hoth_host_request) +
              sizeof(struct hoth_secret_provisioning_request_header));
          EXPECT_EQ(args->offset, 0);
          EXPECT_EQ(args->size, 1008);
        } else {
          EXPECT_EQ(request_size,
                    sizeof(struct hoth_host_request) +
                        sizeof(struct hoth_secret_provisioning_request_header) +
          ...
          const auto* args = reinterpret_cast<
              ...
              sizeof(struct hoth_host_request) +
              sizeof(struct hoth_secret_provisioning_request_header));
          EXPECT_EQ(args->offset, 1008);
          EXPECT_EQ(args->size, 304);
        }
        chunk_index++;
        return LIBHOTH_OK;
      });
  ...
  EXPECT_EQ(libhoth_secret_provisioning_load_mldsa_public_key(
                &hoth_dev_, key.data(), key.size()),
            HOTH_SUCCESS);
}

TEST_F(LibHothTest, LoadMldsaPublicKeyRejectsInvalidSizes) {
  const std::vector<uint8_t> key(
      HOTH_SECRET_PROVISIONING_MLDSA44_PUBLIC_KEY_BYTES, 0x3c);

  EXPECT_EQ(LIBHOTH_ERR_GET_CODE(libhoth_secret_provisioning_load_mldsa_public_key(
                &hoth_dev_, nullptr, key.size())),
            LIBHOTH_ERR_INVALID_PARAMETER);
  EXPECT_EQ(LIBHOTH_ERR_GET_CODE(libhoth_secret_provisioning_load_mldsa_public_key(
                &hoth_dev_, key.data(), key.size() - 1)),
            LIBHOTH_ERR_INVALID_PARAMETER);
  EXPECT_EQ(LIBHOTH_ERR_GET_CODE(libhoth_secret_provisioning_load_mldsa_public_key(
                &hoth_dev_, key.data(), key.size() + 1)),
            LIBHOTH_ERR_INVALID_PARAMETER);
